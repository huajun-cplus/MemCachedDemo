...
public:
     ...
     */
    CReadWriteBuffer(const CReadWriteBuffer &);
     ...
     */
    CReadWriteBuffer & operator=(const CReadWriteBuffer &);
    ...
    */
    ...
    bool operator==(const CReadWriteBuffer &) const;

private:
    size_t  m_siBufSiz;     //!< max buf size
    size_t  m_siBufLen;     //!< committed writes
    size_t  m_siBufIdx;     //!< committed reads
    size_t  m_siGrowBy;     //!< growby rate
    bool    m_bExternal;    //!< is this an external buffer?
    char    *m_pBuf;        //!< buffer
};
